...
export async function updatePerk(req, res, next) {
  try {
    // validate request body against schema, allow unknown fields for partial updates
    const partialPerkSchema = perkSchema.fork(
      Object.keys(perkSchema.describe().keys),
      (schema) => schema.optional()
    );
    const { value, error } = partialPerkSchema.validate(req.body);
    if (error) return res.status(400).json({ message: error.message });
